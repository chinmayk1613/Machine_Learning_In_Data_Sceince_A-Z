# -*- coding: utf-8 -*-
"""
Created on Wed May  6 19:30:49 2020

@author: Chinmay Kashikar
"""

#Upper Confidence Bound Algorithm..lec no 196


#import library
import numpy as np   #cotain maths
import math
import matplotlib.pyplot as plt #help to plot nice chart.To plot something
import pandas as pd #to import dataset and to manage data set

#Importing Dataset
dataset=pd.read_csv('Ads_CTR_Optimisation.csv')

#Implementing UCB
N = 10000 #no of users
d= 10 #no of adds
ads_selected = []
numbers_of_selections = [0] * d
sums_of_rewards = [0] *d
total_reward = 0

for n in range(0,N):
    ad = 0
    max_upper_bound = 0
    for i in range(0,d):
        if (numbers_of_selections[i]>0):
            avearge_reward = sums_of_rewards[i]/numbers_of_selections[i]
            delta_i=math.sqrt(3/2* math.log(n + 1)/numbers_of_selections[i])
            upper_bound=avearge_reward + delta_i
        else:
            upper_bound = 1e400
        if upper_bound > max_upper_bound:
            max_upper_bound=upper_bound
            ad = i
    ads_selected.append(ad)
    numbers_of_selections[ad]=numbers_of_selections[ad] + 1
    reward = dataset.values[n,ad]
    sums_of_rewards[ad]=sums_of_rewards[ad]+reward
    total_reward = total_reward + reward
    

#Visualising Result
plt.hist(ads_selected)    
plt.title('Histrogram Of ads Selection')
plt.xlabel('Ads')
plt.ylabel('Frequency of ads that selected')
plt.show()    
        
